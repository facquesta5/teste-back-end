<?php
    class Basic {
        ...
        public function edit($table, $match, $data, $id){
            ...
            $dados = implode(', ', $dados); 
            $data['id'] = $id;
            $sql = "UPDATE $table SET $dados WHERE $match=:id";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute($data);
        ...
        public function detalhesUsuario($id){
            ...
            cidades.nome as cidade, cidades.id as cidade_id, estados.nome as estado, enderecos.estado_id
            FROM enderecos  
            left join usuarios on 
            usuarios.endereco_id = enderecos.id
            left join cidades on
            enderecos.cidade_id = cidades.id
